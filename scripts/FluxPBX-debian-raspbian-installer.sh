#!/bin/sh
# (C) 2016 Ken Rice <krice@fluxpbx.org>
# Licensed as per the MPL1.1
#
########################################################
# TODO: FluxPBX AutoStart
...
welcome_screen() {
	$DIALOG --title "FluxPBX with LetsEncrypt AutoInstaller" --clear \
		--msgbox "This Script with automattically Install FluxPBX \
...
FluxPBX and Verto Communicator for function properly. \n\n\
...
fs_ver_select() {
	$DIALOG --backtitle "FluxPBX Version" \
		--title "RADIOLIST BOX" --clear \
		--radiolist "Which Version of FluxPBX are you installing? \n" 20 61 5 \
		"1"  "FluxPBX 1.7" ON \
		"2"  "FluxPBX 1.6" off 2> $tempfile
...
config_fs_repos() {
	curl https://files.fluxpbx.org/repo/deb/debian/fluxpbx_archive_g0.pub | apt-key add -
	if [ "$FS_REV" = "master" ]; then
		echo "deb http://files.freeswitch.org/repo/deb/debian-unstable/ jessie main" >/etc/apt/sources.list.d/fluxpbx.list
		REPO="https://github.com/signalwire/fluxpbx.git"
		...
		REPO="-b v1.6 https://github.com/signalwire/fluxpbx.git"
	fi
	apt-get update 2>&1 >/dev/null
}
...
get_fs_source() {
	echo "REPO = $REPO"
	if [ ! -d /usr/src/fluxpbx.git ]; then
		cd /usr/src
		git clone $REPO fluxpbx.git
	else
		cd /usr/src/fluxpbx.git
		git clean -fdx
...
install_certs() {
	...
	# if we dont have the FluxPBX Certs Directory, make it
	if [ $NEED_CERTS_INSTALL -eq 1 ]; then

		if [ ! -d /usr/local/fluxpbx/certs ]; then
			mkdir -p /usr/local/fluxpbx/certs
		fi

		cat /etc/letsencrypt/live/$FQDN/cert.pem /etc/letsencrypt/live/$FQDN/privkey.pem \
			/etc/letsencrypt/live/$FQDN/chain.pem > /usr/local/fluxpbx/certs/wss.pem
	fi
...
build_fs() {
	...
	#if we already have a FluxPBX install from source clean out the old bins
	if [ -d /usr/local/fluxpbx/bin ]; then
		rm -rf /usr/local/fluxpbx/{bin,mod,lib}/*
	fi
	cd /usr/src/fluxpbx.git
	if [ ! -d /usr/src/fluxpbx.git/configure ]; then
		./bootstrap.sh -j
	fi
	./configure -C
...
install_vc() {
	if [ ! -d /usr/src/fluxpbx.git/html5/verto/verto_communicator ]; then
		get_fs_source
	fi
	...
	cd /usr/src/fluxpbx.git/html5/verto/verto_communicator
	apt-get update 
...
fluxpbx_debian_packages() {
	apt-get install -o Dpkg::Progress=1 -y fluxpbx-all fluxpbx-all-dbg gdb 2>&1 | awk -W interactive '/Progress/ { print }'| \
		sed -u 's/[^0-9]//g' | dialog --gauge "Please wait.\n Installing FluxPBX..." 10 70 0
}

fluxpbx_debian_source() {
	apt-get install -o Dpkg::Progress=1 -y fluxpbx-video-deps-most \
