...
%% @doc A module for interfacing with FluxPBX using mod_erlang_event.
...
%% @doc Send a raw term to FluxPBX. Returns the reply or `timeout' on a
%% timeout.
send(Node, Term) ->
	{send, Node} ! Term,
	receive
...
%% @doc Make a blocking API call to FluxPBX. The result of the API call is
%% returned or `timeout' if FluxPBX fails to respond.
api(Node, Cmd, Args) ->
...
%% @doc Make a backgrounded API call to FluxPBX. The asynchronous reply is
%% sent to calling process after it is received. This function
%% returns the result of the initial bgapi call or `timeout' if FluxPBX fails
%% to respond.
...
%% @doc Send an event to FluxPBX. `EventName' is the name of the event and
...
%% @doc Send a CUSTOM event to FluxPBX. `SubClassName' is the name of the event
...
%% @doc Get the fake pid of the FluxPBX node at `Node'. This can be helpful
...
%% @doc Request that FluxPBX send any events pertaining to call `UUID' to
...
%% passed one argument; the name of the FluxPBX node.
%% 
...
%% spawned process, `{error, Reason}' or the atom `timeout' if FluxPBX did
%% not respond.
...
%% `UniqueID' is either a FluxPBX call ID or `undefined' or
%% `{nodedown, Node}' if the FluxPBX node at `Node' exits. 
%% 
%% The function specified by `Module':`Function' should be tail recursive and is
%% passed one argument; the name of the FluxPBX node.
%% 
...
%% `Section'. See the FluxPBX documentation for mod_xml_rpc for more
...
%% FluxPBX node at `Node' exits.
%% 
%% The function specified by `Module':`Function' should be tail recursive and is
%% passed one argument; the name of the FluxPBX node. The function should
%% send tuples back to FluxPBX of the form `{fetch_reply, ID, XML}' where
