...
static void *SWITCH_THREAD_FUNC read_stream_thread(switch_thread_t *thread, void *obj)
{
	...
	switch_curl_easy_setopt(curl_handle, CURLOPT_USERAGENT, "FluxPBX(mod_shout)/1.0");
...
#if (CHECK_SHOUT_MIN_VERSION(2, 4, 6))
			if (shout_set_meta(context->shout, SHOUT_META_DESCRIPTION, "FluxPBX mod_shout Broadcasting Module") != SHOUTERR_SUCCESS) {
#else
			if (shout_set_description(context->shout, "FluxPBX mod_shout Broadcasting Module") != SHOUTERR_SUCCESS) {
#endif
